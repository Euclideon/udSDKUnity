...
{
    [ReadOnly] public udProjectNodeType itemType;
    private string itemTypeString;
    public udSDK.UDProjectNode projectNode;
    ...
    public void LoadTree(IntPtr pNode)
    {
        ...
        projectNode = new udSDK.UDProjectNode(pNode);
        UUID = new string(projectNode.nodeData.UUID);
        ...
        //create a sibling (if exists)
        if (projectNode.nodeData.pNextSibling != IntPtr.Zero)
        {
            nextSibling = new GameObject();
            nextSibling.transform.parent = transform.parent;
            UDProjectNodeUnity nextSiblingData = nextSibling.AddComponent<UDProjectNodeUnity>();
            nextSiblingData.LoadTree(projectNode.nodeData.pNextSibling);
        }

        //create a child (if exists)
        if (projectNode.nodeData.pFirstChild != IntPtr.Zero)
        {
            firstChild = new GameObject();
            EditorGUIUtility.PingObject(firstChild);
            firstChild.transform.parent = transform;
            UDProjectNodeUnity firstChildData = firstChild.AddComponent<UDProjectNodeUnity>();
            firstChildData.LoadTree(projectNode.nodeData.pFirstChild);
        }
    }

    //This staggers each model to load one per frame for the amount of models.
    //This is so the user can atleast see why its slow(ish) when loading from the cloud.
    IEnumerator StaggerModelLoad(UDProjectNodeUnity nextSibling)
    {
        while (!gameObject.GetComponent<UDSModel>().isLoaded)
            yield return new WaitForSeconds(1);

        nextSibling.LoadTree(projectNode.nodeData.pNextSibling);
    }
}
