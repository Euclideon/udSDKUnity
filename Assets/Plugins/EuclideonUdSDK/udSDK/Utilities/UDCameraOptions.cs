...
public class UDCameraOptions : MonoBehaviour
{
    ...
    public bool placeCubesOnPick = false;
    [System.NonSerialized]
      ...
      resolutionScaling = 1;
        ...
        if (optionsStruct.pickRendered)
        {
            ...
            if (placeNext && pick.hit == 0)
            {
                ...
                UDSModel[] modelArray = FindObjectOfType<UDProjectUnity>().GetComponentsInChildren<UDSModel>(includeInactive: true);

                for (int i = 0; i < modelArray.Length; i++)
                    modelArray[modelArray.Length - 1 - pick.modelIndex].isSelected = false;
            }             
            else
            {
                if (placeNext)
                {
                    ...
                    UDSModel[] modelArray = FindObjectOfType<UDProjectUnity>().GetComponentsInChildren<UDSModel>(includeInactive: true);
                    EditorGUIUtility.PingObject(modelArray[modelArray.Length-1-pick.modelIndex]); //change to only iterate through udsmodels.

                    for (int i = 0; i < modelArray.Length; i++)
                    {
                        if(i == modelArray.Length - 1 - pick.modelIndex)
                            modelArray[modelArray.Length - 1 - pick.modelIndex].isSelected = true;
                        else
                            modelArray[i].isSelected = false;
                    }
